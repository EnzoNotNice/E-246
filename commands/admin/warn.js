...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      if (target.user.bot)
        return interaction.reply({ embeds: [error('لا يمكنك تحذير البوتات.')], flags: ['Ephemeral'] });
      ...
      const logEmbed = modlog('تم تحذير العضو', { tag: target.user.tag, id: target.id }, interaction.user, reason, {
        '{emoji:alerttriangle} إجمالي التحذيرات': String(warnings.length)
      });
      await sendLog(interaction.client, interaction.guildId, logEmbed, 'warn');

      return interaction.editReply({
        embeds: [
          success(locale.get('moderation.warnSuccess', { user: target.user.tag, reason, total: warnings.length }))
        ]
      });
    } catch (err) {
      logger.error('[Command Error - warn.js]:', err);
